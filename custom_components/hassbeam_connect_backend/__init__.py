"""
HassBeam Connect Backend Integration for Home Assistant
"""
...
from .database import (
    ...
    get_ir_codes, 
    save_ir_code, 
    check_ir_code_exists, 
    delete_ir_code, 
    get_ir_code_by_device_action
)
...
def _format_codes(codes: List[tuple]) -> List[Dict[str, Any]]:
...
def _parse_array_string(array_str: Any) -> Any:
...
def _prepare_service_data(protocol: str, event_data: Dict[str, Any]) -> tuple[str, Dict[str, Any]]:
    """Prepare service data for the specific protocol."""
    if protocol not in PROTOCOL_MAPPINGS:
        raise ValueError(f"Unsupported protocol: {protocol}")
    
    service_info = PROTOCOL_MAPPINGS[protocol]
    service_data = {}
    ...
    for param in service_info["params"]:
        if param in event_data:
            ...
            # Handle array parameters for specific protocols
            ...
            service_data[param] = DEFAULT_PARAMS[param]
    
    return service_info["service"], service_data


async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up HassBeam Connect Backend from a config entry."""
    _LOGGER.debug("Setting up HassBeam Connect Backend integration")
    ...
    _LOGGER.info("HassBeam Connect Backend integration setup completed successfully")
    return True
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    _LOGGER.debug("Unloading HassBeam Connect Backend integration")
    ...
    _LOGGER.info("HassBeam Connect Backend integration unloaded successfully")
    return True
